        ...
        <div class="card">
            ...
            <div class="card-body">
                <div class="p-2 g-col-6">
                    <!-- Button to trigger add location modal -->
                        ...
                        <tbody>
                            @foreach ($locations as $location)
                                <tr>
                                    ...
                                    <td class="text-center">
                                        ...
                                        <button type="button" class="btn btn-danger" data-toggle="modal"
                                            data-target="#deleteLocationModal{{ $location->id }}">
                                            Delete
                                        </button>
                                    </td>
                                </tr>
                                            ...
                                            <form action="{{ route('locations.update', $location->id) }}" method="POST">
                                                ...
                                                <div class="modal-body">
                                                    <div class="form-group">
                                                        <label for="locationName{{ $location->id }}">location Name</label>
                                                        <input type="text" class="form-control"
                                                            id="locationName{{ $location->id }}" name="name"
                                ...
                                <div class="modal fade" id="deleteLocationModal{{ $location->id }}" tabindex="-1"
                                    role="dialog" aria-labelledby="deleteLocationModalLabel{{ $location->id  }}"
                                    aria-hidden="true">
                                    <div class="modal-dialog" role="document">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                            ...
                                            <form action="{{ route('locations.destroy', $location->id) }}" method="POST">
                                                ...
                                                <div class="modal-body">
                                                    <div class="form-group">
                                                        <label for="reassignLocation{{ $location->id  }}">Reassign items
                                                            to:</label>
                                                        <select class="form-control"
                                                            id="reassignLocation{{ $location->id  }}"
                                                            name="reassign_location_id" required>
                                                            @foreach ($allLocations->where('id', '!=', $location->id) as $reassignLocation)
                                                                <option value="{{ $reassignLocation->id }}">
                                                                    {{ $reassignLocation->name }}
                                                                </option>
                                                            @endforeach
                                                        </select>
                                                    </div>
                                                    <p>Are you sure you want to delete this location? All related items will
                                                        be reassigned to the selected location.</p>
                                                </div>
                                                <div class="modal-footer">
                                                    <button type="button" class="btn btn-secondary"
